...
distanceVertical_AP_RIS = h_RIS - h_BS;
...
pilotIndex = zeros(K);
D = zeros(L,K);                 % Matriz de seleccíon de UE servidos por AP (DCC)
betaLoS_AP_UE = zeros(L,1);     % Atenuación del canal
...
%% Setups
    ...
    for k = 1:K
        ...
        gain_NLoS_AP_UE(probLoS_AP_UE(:,k)==0)=channelGain_AP_UE(probLoS_AP_UE(:,k)==0); 
        ...
        for s = 1:S
            ...
            % Matriz de correlación espacial (modelo de dispersión local)
            if nargin > 6
                R_RIS_UE(:,:,s,k) = gain_NLoS_RIS_UE(s) * functionRlocalscattering3D(N_H_RIS,N_V_RIS,antennaSpacing_RIS,antennaSpacing_RIS,angletoUE_varphi,ASD_varphi,angletoUE_theta,ASD_theta,'Gaussian');
            ...
            % Componente determinista LoS
            a_H_RIS_UE = exp(1i * 2 * pi * antennaSpacing * (0:N_H_RIS - 1).' *sin(angletoUE_theta));
            a_V_RIS_UE = exp(1i * 2 * pi * antennaSpacing * (0:N_V_RIS - 1).' * sin(angletoUE_varphi)*cos(angletoUE_theta));
            arrayResp_RIS_UE = kron(a_H_RIS_UE,a_V_RIS_UE);                                                 % Ecuación 4.129 Introduction to Multiple Antenna Communications and Reconfigurable Surfaces
    ...
    for s = 1:S
        ...
        % Probabilidad aleatoria de LoS si LoS==2
        if LoS==2
            probLoS_AP_RIS(:,s)=1;%(rand<((maxdistances-dist_AP_RIS(:,s))./maxdistances));
        end
        ...
        for l = 1:L
            ...
            a_H_RIS_AP = exp(1i * 2 * pi * antennaSpacing * (0:N_H_RIS - 1).' *sin(angle_theta));
            a_V_RIS_AP = exp(1i * 2 * pi * antennaSpacing * (0:N_V_RIS - 1).' * sin(angle_varphi)*cos(angle_theta));
            arrayResp_RIS = kron(a_H_RIS_AP,a_V_RIS_AP);                                                 % Ecuación 4.129 Introduction to Multiple Antenna Communications and Reconfigurable Surfaces
