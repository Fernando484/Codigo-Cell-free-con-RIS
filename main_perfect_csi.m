...
for s = 1:length(S_values)
    ...
    for n = 1:nbrOfSetups
        ...
        if (groupRIS_size > 1)
            ...
            % Remodelamos a un tensor 6D para sumar los bloques directamente
            tempH = reshape(H_cascade, [L*N_AP, g_V, N_V_RIS/g_V, g_H, N_H_RIS/g_H, S, nbrOfRealizations, K]);
            H_cascade_grouped = reshape(sum(sum(tempH, 2), 4), [L*N_AP, Ngroup*S, nbrOfRealizations, K]);

            tempHMean = reshape(HMean_cascade, [L*N_AP, g_V, N_V_RIS/g_V, g_H, N_H_RIS/g_H, S, nbrOfRealizations, K]);
            HMean_cascade_grouped = reshape(sum(sum(tempHMean, 2), 4), [L*N_AP, Ngroup*S, nbrOfRealizations, K]);
        end
        if (S>0)
            % 3. Vectorizar la reconstrucción con repelem (evitamos 5 bucles anidados)
            if (groupRIS_size == 1)
                [thetaMatrix] = PhaseSelect(nbrOfRealizations,H_AP_UE,H_cascade,risAssignment,S,N_RIS,p,N_AP,L);
            else
                
                % temp_grouped = reshape(H_cascade_grouped, [L*N_AP, N_V_RIS/g_V, N_H_RIS/g_H, S, nbrOfRealizations, K]);
                % temp_rep = repelem(temp_grouped, 1, g_V, g_H, 1, 1, 1);
                % Hhat_cascade_aux = reshape(temp_rep, [L*N_AP, N_RIS*S, nbrOfRealizations, K]);
            ...
            [Hhat_agregated,B_agregated,C_agregated] = channelEstimates(H_cascade,HMean_AP_UE,[],[],[],[],R_cascade,[],nbrOfRealizations,L,K,N_AP,K,pilotIndex,p,risAssignment,S,[]);
...
save('SE_GroupOf16_100Setups1RIS1UEnoPrelog4')
